...
def create_kara_cn_cndtl(cn_no,cn_date,debtor_name,debtor_code,seq,sales_agent,item_code,quantity,uom,discount_amount,net_amount,price,temporary_display_term,tempcompanyautokey,branchautokey,location_instance,description, reason, our_invoice):
    ...
    if not filter_cn_number:
        ...
        if not filter_cn_cndtl:
            ...
            get_cndtl = CNDTL.objects.get(headerautokey=get_cn_object,seq=seq)
            new_value = get_cn_object.total - get_cndtl.subtotal + Decimal(net_amount).quantize(Decimal('0.0000'))
            get_cn_object.total = new_value
            get_cn_object.nettotal = new_value
            get_cn_object.localnettotal = new_value
            get_cn_object.analysisnettotal = new_value
