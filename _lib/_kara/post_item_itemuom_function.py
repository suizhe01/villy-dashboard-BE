...
def create_kara_item_itemuom_invoice(item_code, description, uom, price, rate, unit_uom, unit_price, unit_rate):
    try:
        ...
        if not filter_itemcode:
            save_new_item = Item(itembrand='kara',itemcode=item_code,description=description,companyautokey=temcompanyautokey,mainsupplier=mainsupplier,dockkey=1,dutyrate=1,costingmethod=0,lastmodified=date_time_now,lastupdate=0)
            save_new_item.save()
