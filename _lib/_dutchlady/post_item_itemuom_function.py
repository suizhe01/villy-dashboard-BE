...
def create_dutchlady_item_itemuom_invoice(item_code, description, uom, price, rate, unit_uom, unit_price, unit_rate):
    try:
        ...
        mainsupplier = 'Dutch Lady'
        date_time_now = datetime.now()

        if not filter_itemcode:
            save_new_item = Item(itembrand='dutchlady',itemcode=item_code,description=description,companyautokey=temcompanyautokey,mainsupplier=mainsupplier,dockkey=1,dutyrate=1,costingmethod=0,lastmodified=date_time_now,lastupdate=0)
            save_new_item.save()
...
def create_dutchlady_item_itemuom_cn(item_code, description, uom, price, rate, unit_uom, unit_price, unit_rate):
    try:
        temcompanyautokey = Company.objects.filter(name='Villy').first()
        if not temcompanyautokey:
            raise ValueError("Company 'Villy' not found")
